...
#else
		WIFIsrl.println(cmd_tbl[cmd].cmd_str);
#endif
		break;
	}
...
#else
		WIFIsrl.write((uint8_t)0x1b);    // data start
		WIFIsrl.write((uint8_t)0x53);
		WIFIsrl.write((uint8_t)int_to_hex(this->client_cid));  // connection ID
#endif
		if (len == 1){
			if (buf[0] != '\r' && buf[0] != '\n'){ 
#if defined(__32MX795F512L__)		// it's a chipKIT Max32
...
#else
				WIFIsrl.write(buf[0]);           // data to send
#endif
			} else if (buf[0] == '\n') {
#if defined(__32MX795F512L__)		// it's a chipKIT Max32
				Serial1.print("\n\r");           // new line
...
#else
		WIFIsrl.write((uint8_t)0x1b);    // data end
		WIFIsrl.write((uint8_t)0x45);		
#endif
	}
	delay(10);
...
void GSClass::flush()
{
	// arduino-1.0 repurposed the WIFIsrl.flush() command
